...
public class App {
    ...
    public static void main(String[] args) throws IOException, URISyntaxException {
        ...
        File coreJarFile = null;
        {
            ...
            File coreJar0 = new File(dataDirPath, "BilibiliDown-1.0-SNAPSHOT.jar");
            // launch.jar 同级目录下的jar
            File launchJarFile = new File(pd.getCodeSource().getLocation().toURI());
            File coreJar1 =
                    new File(launchJarFile.getParentFile(), "BilibiliDown-1.0-SNAPSHOT.jar");
            // 取最新的那个jar
            if (coreJar0.exists() && coreJar1.exists())
                coreJarFile =
                        coreJar0.lastModified() > coreJar1.lastModified() ? coreJar0 : coreJar1;
            else coreJarFile = coreJar0.exists() ? coreJar0 : coreJar1;
        }

        MemoryURLHandler.addSource("BilibiliDown-1.0-SNAPSHOT.jar", readAllBytes(coreJarFile));
    ...
    /**
     * Restart the current Java application See
    ...
    public static void restartApplication() throws IOException {
        try {
            ...
            if (jarPath != null) {
                ...
                cmd.append(
                        "-cp \"" + System.getProperty("java.class.path") + "\" " + mainCommand[0]);
            }
