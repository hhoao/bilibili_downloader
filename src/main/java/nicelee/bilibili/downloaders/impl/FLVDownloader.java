...
public class FLVDownloader implements IDownloader {
    ...
    private String tryReplaceHost(String url) {
        if (Global.forceReplaceUposHost) {
            if (hostPattern == null) {
                ...
                // 不能替换
                // https://xy111x48x218x66xy.mcdn.bilivideo.cn:8082/v1/resource/1597612179-1-30280.m4s
                // 可以替换
                // https://xy111x48x218x66xy.mcdn.bilivideo.cn:8082/upgcxcode/35/83/1532588335/1532588335-1-30280.m4s
    ...
    protected boolean download(String url, String avId, int qn, int page, String suffix) {
        ...
        if (url.contains("#")) {
            ...
            if (util.getStatus() == StatusEnum.STOP) return false;
        ...
        if (ok) {
            ...
            if (Global.alertIfFFmpegFail)
                throw new BilibiliError(
                        "如需关闭该警告，请在配置页搜索并修改配置 bilibili.alert.ffmpegFail\n\t转码失败，请检查ffmpeg配置: "
                                + msg);
            else return false;
        }
    }
