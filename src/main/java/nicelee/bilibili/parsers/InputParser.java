...
public class InputParser implements IInputParser, IParamSetter {
    ...
    protected static final Pattern paramPattern =
            Pattern.compile("^(.*)p=([0-9]+)$"); // 自定义参数, 目前只匹配个人主页视频的页码
                    ...
                    try {
                        for (Class<?> clazz : PackageScanLoader.validParserClasses) {
                            ...
                            if (bili.ifLoad().isEmpty() || bili.ifLoad().equals("listAll")) {
                                ...
                                Constructor<IInputParser> con =
                                        (Constructor<IInputParser>) clazz.getConstructors()[0];
                                IInputParser inputParser =
                                        con.newInstance(
                                                new Object[] {new Object[] {null, null, 0}});
        ...
        for (IInputParser parser : parsers) {
            if (parser.matches(input)) {
                try {
                    ...
                    this.parser =
                            (IInputParser)
                                    parser.getClass().getConstructors()[0].newInstance(param);
