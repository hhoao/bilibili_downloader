...
public abstract class AbstractBaseParser implements IInputParser {
    ...
    public VideoInfo getAVDetail(String bvId, int videoFormat, boolean getVideoLink) {
        ...
        String url =
                String.format(
                        "https://api.bilibili.com/x/player/pagelist?bvid=%s&jsonp=jsonp", bvId);
        HashMap<String, String> headers_json = new HttpHeaders().getBiliJsonAPIHeaders(bvId);
        String json =
                util.getContent(url, headers_json, HttpCookies.globalCookiesWithFingerprint());
        Logger.println(url);
        Logger.println(json);
        ...
        //		String detailUrl =
        // String.format("https://api.bilibili.com/x/web-interface/view?cid=%d&bvid=%s", cid, bvId);
        //		String detailJson = util.getContent(detailUrl, headers_json,
        // HttpCookies.globalCookiesWithFingerprint());
        //		JSONObject detailObj = new JSONObject(detailJson).getJSONObject("data");
        String detailUrl =
                "https://api.bilibili.com/x/web-interface/wbi/view/detail?platform=web&page_no=1&p=1&need_operation_card=1&web_rm_repeat=1&need_elec=1&bvid="
                        + bvId;
        detailUrl += API.genDmImgParams();
        detailUrl = API.encWbi(detailUrl);
        String detailJson =
                util.getContent(
                        detailUrl, headers_json, HttpCookies.globalCookiesWithFingerprint());
        ...
        if (detailRaw.optInt("code") == -403) {
            detailUrl =
                    String.format(
                            "https://api.bilibili.com/x/v3/fav/resource/infos?resources=%d:2&platform=web&folder_mid=&folder_id=",
                            aid);
            detailJson =
                    util.getContent(
                            detailUrl, headers_json, HttpCookies.globalCookiesWithFingerprint());
        ...
        if (videoCnt > 1 && array.length() == 1) {
            // 查询graph_version版本
            String url_graph_version =
                    String.format(
                            "https://api.bilibili.com/x/player/wbi/v2?aid=%d&cid=%d&isGaiaAvoided=false",
                            aid, cid);
            ...
            String graph_version =
                    new JSONObject(result)
                            .getJSONObject("data")
                            .getJSONObject("interaction")
                            .optString("graph_version");
            Logger.println(graph_version);
            List<List<StoryClipInfo>> story_list = new ArrayList<>();
            List<StoryClipInfo> originStory = new ArrayList<>();
            ...
            LinkedHashMap<Long, ClipInfo> clipMap =
                    storyList2Map(bvId, videoFormat, getVideoLink, viInfo, story_list);
                ...
                try {
                    ...
                    for (int qn : qnList) {
                        if (getVideoLink) {
                            String link =
                                    getVideoLink(
                                            bvId, String.valueOf(clip.getcId()), qn, videoFormat);
                            links.put(qn, link);
                        } else {
                            links.put(qn, "");
                        }
    ...
    private int[] getVideoQNList_JudgeTypeFirst(String bvId, String cid) {
        ...
        String url =
                "https://api.bilibili.com/x/web-interface/wbi/view/detail?platform=web&bvid="
                        + bvId;
        ...
        JSONObject infoObj = new JSONObject(callBack).getJSONObject("data").getJSONObject("View");
        Long aid = infoObj.optLong("aid");

        if (infoObj.optString("redirect_url").isEmpty()) {
            // 普通类型
            url =
                    "https://api.bilibili.com/x/player/playurl?cid=%s&bvid=%s&qn=%d&type=&otype=json&fnver=0&fnval=4048&fourk=1";
            url = String.format(url, cid, bvId, 32);
            Logger.println(url);
            String json =
                    util.getContent(
                            url,
                            headers.getBiliJsonAPIHeaders(bvId),
                            HttpCookies.globalCookiesWithFingerprint());
            Logger.println(json);
            ...
            // 非普通类型
            url =
                    "https://api.bilibili.com/pgc/player/web/playurl?fnval=4048&fnver=0&fourk=1&otype=json&avid=%s&cid=%s&qn=%s";
            url = String.format(url, aid, cid, 32);
            Logger.println(url);
            String json =
                    util.getContent(
                            url,
                            headers.getBiliJsonAPIHeaders("av" + aid),
                            HttpCookies.globalCookiesWithFingerprint());
            Logger.println(json);
    ...
    private int[] getVideoQNList_TryNormalTypeFirst(String bvId, String cid) {
        ...
        try {
            ...
            long aid = ConvertUtil.Bv2Av(bvId);
            String url =
                    "https://api.bilibili.com/pgc/player/web/playurl?fnval=4048&fnver=0&fourk=1&otype=json&avid=%d&cid=%s&qn=%s";
            url = String.format(url, aid, cid, 32);
            Logger.println(url);
            String json =
                    util.getContent(
    ...
    protected String getAudioLink(String auId, String _auId, int qn) {
        ...
        //		String url =
        // String.format("https://www.bilibili.com/audio/music-service-c/web/url?sid=%s&privilege=2&quality=2", auIdNum);
        String url =
                String.format(
                        "https://www.bilibili.com/audio/music-service-c/url?songid=%s&privilege=2&quality=%d&mid=&platform=web",
                        auIdNum, qn);
        Logger.println(url);
    ...
    protected String getVideoSubtitleLink(String bvId, String cid, int qn) {
        String url =
                String.format(
                        "https://api.bilibili.com/x/player/wbi/v2?bvid=%s&cid=%s&isGaiaAvoided=false",
                        bvId, cid);
        url += API.genDmImgParams();
        url = API.encWbi(url);
        Logger.println(url);
    ...
    String getVideoLinkByFormat(String bvId, String cid, int qn, int downloadFormat) {
        ...
        if (detailRaw.optInt("code") == -403) {
            ...
            String detailUrl =
                    String.format(
                            ...
                            aid);
            String detailJson =
                    util.getContent(detailUrl, header, HttpCookies.globalCookiesWithFingerprint());
            Logger.println(detailUrl);
            Logger.println(detailJson);
            JSONObject detailObj = new JSONObject(detailJson).getJSONArray("data").getJSONObject(0);
            isNormalType = detailObj.getInt("attr") != 2; // 0 普通 16 互动视频
        } else {
            JSONObject infoObj = detailRaw.getJSONObject("data").getJSONObject("View");
        ...
        if (isNormalType) {
            ...
            url = API.encWbi(url);
            Logger.println(url);
            //			List cookie = downloadFormat == 2 ? null :
            // HttpCookies.globalCookiesWithFingerprint();
            ...
            // 非普通类型
            url =
                    "https://api.bilibili.com/pgc/player/web/playurl?fnver=0&fourk=1&otype=json&avid=%s&cid=%s&qn=%s&fnval=%s";
     ...
     */
    private void collectStoryList(
            String bvid,
            String node,
            String graph_version,
            List<StoryClipInfo> currentStory,
            List<List<StoryClipInfo>> story_list,
            HashSet<StoryClipInfo> node_list) {
        ...
        String url_node_format =
                "https://api.bilibili.com/x/stein/edgeinfo_v2?bvid=%s&edge_id=%s&graph_version=%s&platform=pc&portal=0&screen=0";
        ...
        String str_nodeInfo =
                util.getContent(url_node, headers_gv, HttpCookies.globalCookiesWithFingerprint());
        ...
        if (questions == null) { // 如果没有选择，则到达末尾，则故事线完整，可以收集
            ...
            for (int i = 0; i < choices.length(); i++) {
                ...
                if (node_list.contains(sClip)) {
                    ...
                    List<StoryClipInfo> cloneStory =
                            new ArrayList<>(currentStory); // 确保不会对传入的故事线产生影响，而是生成新的时间线
            ...
            if (is2Added) story_list.add(currentStory);
        }
    }

    String getUrlOfMedia(
            JSONObject media, boolean checkValid, HashMap<String, String> headerForValidCheck) {
    ...
    JSONObject findMediaByPri(List<JSONObject> medias, int priority, int mediaType) {
        for (JSONObject media : medias) {
            if (-1 == priority
                    || (mediaType == 0 && media.getInt("codecid") == priority)
                    || (mediaType == 1 && media.getInt("id") == priority)) return media;
        }
        return null;
    }
    // All 			视频链接#音频链接
    // VideoOnly 	视频链接#
    // AudioOnly	#音频链接
    protected String parseType1(
            JSONObject jObj, int linkQN, HashMap<String, String> headerForValidCheck) {
        ...
        if (Global.downloadMode == DownloadModeEnum.AudioOnly) {
            ...
            if (qnVideos.size() == 0) {
                ...
                System.out.println(
                        "API返回质量为:" + linkQN + "的链接, 实际上只有质量为:" + paramSetter.getRealQN() + "的链接");
                qnVideos.add(v);
            }
            // 根据清晰度选择合适的编码优先级
            Integer rQN = Integer.valueOf(paramSetter.getRealQN());
            int[] videoCodecPriority =
                    Global.videoCodecPriorityMap.getOrDefault(rQN, Global.videoCodecPriority);
            // 根据需求选择编码合适的视频
     ...
     * @return
     */
    private LinkedHashMap<Long, ClipInfo> storyList2Map(
            String bvId,
            int videoFormat,
            ...
            for (int j = 0; j < story_clips.size(); j++) {
                ...
                if (clip_t == null) {
                    ...
                    if (i == 0)
                        clip.setTitle(
                                String.format(
                                        "%s_%d.%d-%s", clip.getTitle(), i, j, obj.getOption()));
