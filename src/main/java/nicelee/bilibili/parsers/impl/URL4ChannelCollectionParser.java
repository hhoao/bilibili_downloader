...
/** 针对的是如下类型的url https://space.bilibili.com/593987248/channel/collectiondetail?sid=508765 */
...
public class URL4ChannelCollectionParser extends AbstractPageQueryParser<VideoInfo> {
    ...
    protected boolean query(int page, int min, int max, Object... obj) {
        ...
        try {
            String urlFormat =
                    "https://api.bilibili.com/x/polymer/space/seasons_archives_list?mid=%s&season_id=%s&sort_reverse=false&page_num=%d&page_size=%d";
            String url = String.format(urlFormat, spaceID, sid, page, API_PMAX);
            String json =
                    util.getContent(url, new HttpHeaders().getCommonHeaders("api.bilibili.com"));
            Logger.println(url);
            ...
            for (int i = min - 1; i < arr.length() && i < max; i++) {
                ...
                try {
                    ...
                    LinkedHashMap<Long, ClipInfo> avClips =
