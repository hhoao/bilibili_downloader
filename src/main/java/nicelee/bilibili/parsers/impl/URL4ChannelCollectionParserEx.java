 ...
 *
 * <p>若是不从网页里面扣，可试一试下面两个api，以后有时间再做 -
 ...
 */
@Bilibili(
        name = "URL4ChannelCollectionParserEx",
        weight = 77,
        ifLoad = "listAll",
        note = "UP 某合集的视频解析")
public class URL4ChannelCollectionParserEx extends AbstractPageQueryParser<VideoInfo> {
    ...
    private static final Pattern pattern =
            Pattern.compile(
                    "space\\.bilibili\\.com/[0-9]+/channel/collectiondetail\\?sid=([0-9]+)");
    private static final Pattern pattern2 =
            Pattern.compile(
                    "space\\.bilibili\\.com/[0-9]+/favlist\\?.*fid=([0-9]+).*ftype=collect");
    private static final Pattern pattern3 =
            Pattern.compile("space\\.bilibili\\.com/[0-9]+/lists/([0-9]+)\\?type=season");
    private String sid;
    ...
    protected boolean query(int page, int min, int max, Object... obj) {
        ...
        try {
            ...
            // 第一个BV的页面里面有所有的信息
            // String urlFormat =
            // "https://api.bilibili.com/x/space/fav/season/list?season_id=%s&pn=%d&ps=%d&jsonp=jsonp";
            // String url = String.format(urlFormat, sid, 1, 1);
            String urlFormat =
                    "https://api.bilibili.com/x/polymer/web-space/seasons_archives_list?mid=%s&season_id=%s&sort_reverse=false&page_num=%d&page_size=%d";
