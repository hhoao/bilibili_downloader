...
/**
 * 针对以下类型的url解析 过时，网页不再出现 https://space.bilibili.com/378034/channel/seriesdetail?sid=918669
 * 过时，网页不再出现
 ...
 *
 * <p>这种方法比URL4ChannelSeriesParser要好一点，因为不需要为每一个BV再进行一次查询
 *
 * <p>查信息 https://api.bilibili.com/x/v1/medialist/info?type=5&biz_id=918669&tid=0 查BV详情 先查询oid
 ...
 */
@Bilibili(
        name = "URL4ChannelSeriesMedialistParser",
        weight = 78,
        ifLoad = "listAll",
        note = "UP 某视频列表的视频解析")
    ...
    protected boolean query(int page, int min, int max, Object... obj) {
        ...
        try {
            ...
            if (pageQueryResult.getVideoName() == null) {
                String url =
                        "https://api.bilibili.com/x/v1/medialist/info?type=5&tid=0&biz_id=" + sid;
                Logger.println(url);
            ...
            if (page > 1) {
                ...
                oid =
                        new JSONObject(json)
                                ...
                                .optString("aid");
                System.out.printf(
                        "page: %d, lastPageNumber: %d, oid: %s\n", page, lastPageNumber, oid);
            }

            // 查询详细信息
            String urlFormat =
                    "https://api.bilibili.com/x/v2/medialist/resource/list?type=5&oid=%s&otype=2&biz_id=%s&with_current=true&mobi_app=web&ps=%s&direction=false&sort_field=1&tid=0&desc=true";
