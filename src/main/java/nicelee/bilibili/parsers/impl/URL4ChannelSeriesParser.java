...
/**
 * 针对以下类型的url解析 https://space.bilibili.com/378034/channel/seriesdetail?sid=918669
 *
 * <p>分页查询的每页个数不宜过多，因为会为每一个BV进行一次查询。这样做是为了确保多P视频的完整 请务必不要在短时间内新建大量此类Tab页，容易被BAN
 *
 * <p>另一个思路是: 查信息 https://api.bilibili.com/x/v1/medialist/info?type=5&biz_id=918669&tid=0 查BV详情
 * 但是这里分页以后，并不是根据page number来查的，而是根据oid(上一次查询最后的视频id)，这不好实现
