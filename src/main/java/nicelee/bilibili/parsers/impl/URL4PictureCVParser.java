 ...
 * 下载专栏里面的图片
 *
 * <p>https://www.bilibili.com/read/cv23435927/?from=readlist
 *
 * <p>https://www.bilibili.com/read/mobile?id=23435927
 */
@Bilibili(name = "URL4PictureCVParser", note = "图片解析 - 专栏")
public class URL4PictureCVParser extends AbstractBaseParser {

    private static final Pattern pattern =
            Pattern.compile("\\.bilibili\\.com/read/(mobile\\?id=|cv)([0-9]+)");
    ...
    protected static final HashMap<String, String> headers =
            new HttpHeaders().getCommonHeaders("www.bilibili.com");
    ...
    protected VideoInfo getCVDetail(String cvIdNumber) {
        ...
        for (int i = 0; i < jModules.length(); i++) {
            ...
            if (mType.equals("MODULE_TYPE_AUTHOR")) jUp = module.getJSONObject("module_author");
                ...
                jTopPics =
                        module.getJSONObject("module_top")
            ...
            if (jUp != null
                    && jParagraphs != null
                    && jTopPics != null
                    && viInfo.getVideoName() != null) break;
        }
        // 总体大致信息
                ...
                for (int nIdx = 0; nIdx < nodes.length(); nIdx++) {
                    ...
                    if ("TEXT_NODE_TYPE_WORD".equals(node.getString("type"))) {
                        ...
                        if (viInfo.getVideoName() == null) {
                            ...
                            if (videoName.length() > 15) videoName = videoName.substring(0, 15);
        ...
        if (viInfo.getVideoName() == null) viInfo.setVideoName("空");
        ...
        for (int i = 0; i < jParagraphs.length(); i++) {
            ...
            if (paraType == 2) {
                ...
                for (int nIdx = 0; nIdx < pics.length(); nIdx++) {
                    ...
                    ClipInfo clip =
                            newCommonClip(cvIdStr, viInfo, author, authorId, cTime, null, null);
                    setPicOfClip(clip, clipMap, picIndex, picUrl);
                    if (viInfo.getVideoPreview() == null) viInfo.setVideoPreview(picUrl);
                    picIndex++;
                }
    ...
    protected void setPicOfClip(
            ClipInfo clip, LinkedHashMap<Long, ClipInfo> clipMap, int picIndex, String picUrl) {
    ...
    protected ClipInfo newCommonClip(
            String cvIdStr,
            VideoInfo viInfo,
            String author,
            String authorId,
    ...
    //		String url =
    // "https://api.bilibili.com/x/article/view?gaia_source=main_web&web_location=333.976&id="
    //				+ cvIdNumber;
    //		url = API.encWbi(url);
    ...
    //		String json = util.getContent(url, headers_json,
    ...
    //		if (opus != null) {
    //			JSONArray jParas =
    // jObj.getJSONObject("opus").getJSONObject("content").getJSONArray("paragraphs");
