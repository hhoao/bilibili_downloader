 ...
 * 下载号主自己的图文收藏（需要登录）
 *
 * <p>https://space.bilibili.com/{spaceID}/favlist?fid=opus
 *
 * <p>https://space.bilibili.com/{spaceID}/favlist?fid=albumfav
 */
...
public class URL4PictureFavParser extends AbstractPageQueryParser<VideoInfo> {
    ...
    private static final Pattern pattern =
            Pattern.compile("space\\.bilibili\\.com/([0-9]+)/favlist\\?fid=(albumfav|opus)");
    ...
    protected boolean query(int page, int min, int max, Object... obj) {
        try {
            String urlFormat =
                    "https://api.bilibili.com/x/polymer/web-dynamic/v1/opus/favlist?page=%d&page_size=%d&timezone_offset=-480";
            ...
            for (int i = min - 1; i < items.length() && i < max; i++) {
                ...
                if (item.optBoolean("is_expired")) continue;
