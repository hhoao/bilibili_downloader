 ...
 * 下载专栏文集里面的图片
 *
 * <p>https://www.bilibili.com/read/mobile-readlist/rl716664
 *
 * <p>https://www.bilibili.com/read/readlist/rl716666
 */
...
public class URL4PictureRLParser extends AbstractPageQueryParser<VideoInfo> {
    ...
    private static final Pattern pattern =
            Pattern.compile("\\.bilibili\\.com/read/[^/]*readlist/rl([0-9]+)");
    ...
    //
    //		String url = "https://api.bilibili.com/x/article/list/web/articles?jsonp=jsonp&id=" +
    // rlIdNumber;
    ...
    public VideoInfo result(int pageSize, int page, Object... obj) {
        ...
        try {
            // 总体大致信息
            Logger.printf(
                    "URL4PictureRLParser正在获取结果: rl%s, p=%d, pageSize=%d",
                    rlIdNumber, page, pageSize);
            String rlIdStr = "rl" + rlIdNumber;

            String url =
                    "https://api.bilibili.com/x/article/list/web/articles?jsonp=jsonp&id="
                            + rlIdNumber;
            HashMap<String, String> headers_json = new HttpHeaders().getCommonHeaders();
            String json =
