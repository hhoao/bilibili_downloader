 ...
 * https://space.bilibili.com/378034/ https://space.bilibili.com/378034/video
 ...
 *
 * <p>sort_field= 最新发布 pubtime, 最多播放 play, 最多收藏 fav 对应api1 order= 最新发布 pubdate, 最多播放 click, 最多收藏
 * stow 对应api2 sort_field= 最新发布 1 , 最多播放 2 , 最多收藏 3 tid // 全部 0, 音乐 3...
 */
@Bilibili(
        name = "URL4UPAllMedialistParser",
        weight = 70,
        ifLoad = "listAll",
        note = "个人上传的视频列表(Medialist解析方式)")
public class URL4UPAllMedialistParser extends AbstractPageQueryParser<VideoInfo> {
    // 针对
    // https://www.bilibili.com/medialist/play/378034?from=space&business=space&sort_field=play&tid=3
    private static final Pattern pattern =
            Pattern.compile("www\\.bilibili\\.com/medialist/play/([0-9]+)\\?.*&business=space&");
    ...
    private static final Pattern pattern2 =
            Pattern.compile("space\\.bilibili\\.com/([0-9]+)(/video|/search/video\\?|/? *$|/?\\?)");
    ...
    private static final String[][] paramDicts =
            new String[][] {{"pubtime", "pubdate"}, {"play", "click"}, {"fav", "stow"}};
    ...
    public boolean matches(String input) {
        ...
        if (matcher.find()) {
            ...
            while (m.find()) params.put(m.group(1), m.group(2).trim());
            return true;
        }
        // 尝试匹配2
        matcher = pattern2.matcher(input);
        if (matcher.find()) {
            if (patternKeyNotEmpty.matcher(input).find()) return false;
    ...
    public VideoInfo result(int pageSize, int page, Object... obj) {
        ...
        try {
            ...
            HashMap<String, String> headers =
                    new HttpHeaders().getCommonHeaders("api.bilibili.com");
            ...
            if (pageQueryResult.getVideoName() == null) {
                String url =
                        "https://api.bilibili.com/x/v1/medialist/info?type=1&tid=0&biz_id="
                                + spaceID;
                Logger.println(url);
            ...
            if (firstOid.equals("end")) return pageQueryResult;
            String lastOidPlus1 = position2Oid(page * pageSize + 1, headersRefer, sortFieldParam);

            // 根据oid查询分页的详细信息
            String urlFormat =
                    "https://api.bilibili.com/x/v2/medialist/resource/list?type=1&oid=%s&otype=2&biz_id=%s&bvid=&with_current=%s&mobi_app=web&ps=%d&direction=false&sort_field=%d&tid=%s&desc=true";
            ...
            while (!findLastOid) {
                ...
                for (int i = 0; i < arr.length(); i++) {
                    ...
                    for (int pointer = 0; pointer < jClips.length(); pointer++) {
                        ...
                        clip.setListOwnerName(
                                pageQueryResult.getAuthor().replaceAll("[/\\\\]", "_"));
    ...
    /**
     * 返回 "" 表示开头 返回 "end" 表示结尾 返回 数字 表示具体id
     *
     * @param pageNumber
     * @param headers
     * @param sortFieldParam
     */
    private String position2Oid(
            int pageNumber, HashMap<String, String> headers, String sortFieldParam) {
        if (pageNumber == 1) return "";
        // String urlFormat =
        // "https://api.bilibili.com/x/space/arc/search?mid=%s&ps=%d&tid=%s&pn=%d&keyword=&order=%s&jsonp=jsonp";
        String urlFormat =
                "https://api.bilibili.com/x/space/wbi/arc/search?mid=%s&ps=%d&tid=%s&special_type=&pn=%d&keyword=&order=%s&platform=web"; // &web_location=1550101&order_avoided=true
        String url =
                String.format(urlFormat, spaceID, 1, params.get("tid"), pageNumber, sortFieldParam);
