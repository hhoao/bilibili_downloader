...
/**
 * 针对以下url类型 https://space.bilibili.com/378034/search/video?tid=3&keyword=葫芦丝&order=pubdate
 * keyword必须不为空 另外，以下类型也可以解析，但是优先级不如URL4UPAllMedialistParser，所以被拦截 https://space.bilibili.com/378034
 * https://space.bilibili.com/378034/ https://space.bilibili.com/378034?spm=xxx
 * https://space.bilibili.com/378034/?spm=xxx https://space.bilibili.com/378034/video
...
public class URL4UPAllParser extends AbstractPageQueryParser<VideoInfo> {
    ...
    protected boolean query(int page, int min, int max, Object... obj) {
        ...
        try {
            // String urlFormat =
            // "https://space.bilibili.com/ajax/member/getSubmitVideos?mid=%s&pagesize=%d&tid=0&page=%d&keyword=&order=pubdate";
            // String urlFormat =
            // "https://api.bilibili.com/x/space/arc/search?mid=%s&ps=%d&tid=%s&pn=%d&keyword=%s&order=%s&jsonp=jsonp";
            String urlFormat =
                    "https://api.bilibili.com/x/space/wbi/arc/search?mid=%s&ps=%d&tid=%s&pn=%d&keyword=%s&order=%s&platform=web"; // &web_location=1550101&order_avoided=true
            ...
            url = API.encWbi(url);
            HashMap<String, String> headersRefer =
            ...
            String json =
                    util.getContent(url, headersRefer, HttpCookies.globalCookiesWithFingerprint());
            Logger.println(url);
            Logger.println(json);
            ...
            for (int i = min - 1; i < arr.length() && i < max; i++) {
                ...
                if (jumpUrl.startsWith("https://www.bilibili.com/cheese/")) continue;
                map.putAll(
                        convertVideoToClipMap(
                                jAV.getString("bvid"),
                                (page - 1) * API_PMAX + i + 1,
                                videoFormat,
                                getVideoLink));
     ...
     */
    private LinkedHashMap<Long, ClipInfo> convertVideoToClipMap(
            String bvid, int remark, int videoFormat, boolean getVideoLink) {
