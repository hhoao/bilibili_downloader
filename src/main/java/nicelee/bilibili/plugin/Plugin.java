...
public class Plugin {
    ...
    public boolean isToCompile(String clazzName) {
        ...
        if (classF.exists()) {
            ...
            if (timeLastModifiedClass > timeLastModifiedJava) return false;
            else return true;
        } else {
            return true;
        }
    ...
    public boolean compile(String clazzName) {
        try {
            ...
            classpath =
                    ResourcesUtil.sourceOf("INeedBiliAV.jar").getCanonicalPath()
     ...
     */
    public Class<?> loadClass(CustomClassLoader ccloader, String clazzName)
