...
public class CmdUtil {
     ...
     * @return
     */
    static final Pattern cmdTxtPattern =
            Pattern.compile("^(av[0-9]+-[0-9]+-p[0-9]+\\.(flv|mp4))\\.txt$");

    static final Pattern cmdDonePartPattern =
            Pattern.compile("^av[0-9]+-[0-9]+-p[0-9]+-part[0-9]+\\.(flv|mp4)$");
    static final Pattern cmdPartPattern = Pattern.compile("^(.*)\\.part$");
    static final Pattern standardFileNamePattern =
            Pattern.compile("^av[0-9]+-[0-9]+-p[0-9]+\\.(flv|mp4)$");
                ...
                new FilenameFilter() {
                    ...
                    public boolean accept(File dir, String name) {
                        ...
                        if (Global.restrictTempMode) {
                            ...
                            // matcherDonePart.group().replaceFirst("-part[0-9]+", ""));
                            //						if (file.exists()) {
                            //							return true;
                            //						}
                            //						return false;
    ...
    private static String replParams(
            String pattern, String videoName, String audioName, String dstName) {
        if (audioName == null) audioName = "null";
        if (videoName == null) videoName = "null";
        return pattern.replace("{FFmpeg}", FFMPEG_PATH)
                .replace("{SavePath}", Global.savePath)
        ...
        if (audioName == null) {
            cmd =
                    new String[] {
                        FFMPEG_PATH,
                        "-i",
                        Global.savePath + videoName,
                        "-c",
                        "copy",
                        Global.savePath + dstName
                    };
        } else if (videoName == null) {
            // cmd = new String[]{ FFMPEG_PATH, "-i", Global.savePath + audioName, "-vn", "-c:a",
            ...
            //			cmd = new String[]{ FFMPEG_PATH, "-i", Global.savePath + videoName, "-i",
            // Global.savePath + audioName, "-c",
            //					"copy", Global.savePath + dstName };
    ...
    public static synchronized void convertOrAppendCmdToRenameBat(
            final String avid_q, final String formattedTitle, int page) {
        try {
            // 获取已完成文件
            ...
            if (Global.doRenameAfterComplete) {
                ...
                if ((!originFile.renameTo(file))
                        && Global.autoNumberWhenFileExists) { // 如果不成功，大概率是文件名重复，在后面加上序号，类似于(01)
                    for (int i = 1; i < 100; i++) {
                        File f =
                                new File(
                                        Global.savePath,
    ...
    static String[] suffixs = {
        ".mp4", ".flv", ".jpg", ".webp", ".png", ".srt", ".ass", ".m4a", ".flac"
    };
    ...
    public static String genFormatedName(
            String avId,
            String pAv,
            String pDisplay,
            int qn,
