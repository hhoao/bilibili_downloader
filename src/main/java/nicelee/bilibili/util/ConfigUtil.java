...
public class ConfigUtil {
    static final Pattern patternConfig =
    ...
    public static void initConfigs() {
        // 先初始化默认值
        try (BufferedReader buReader =
                new BufferedReader(
                        new InputStreamReader(
                                ConfigUtil.class.getResourceAsStream("/app.config"),
    ...
    public static boolean saveConfig() {
        ...
        try (BufferedWriter buWriter =
                new BufferedWriter(new OutputStreamWriter(new FileOutputStream(tmp), "utf-8"))) {
            ...
            for (Entry<String, String> entry : notSaveYet.entrySet()) {
                if (!entry.getValue().isEmpty()
                        && !Global.settingsMustCreateManualy.contains(entry.getKey())) {
