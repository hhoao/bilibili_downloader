...
public class HttpRequestUtilEx extends HttpRequestUtil {
    ...
    public boolean download(String url, String fileName, HashMap<String, String> headers) {
        ...
        if (totalFileSize == -1) {
            ...
            return super.download(url, fileName, headers);
        } else if (Global.multiThreadMinFileSize != 0
                && totalFileSize <= Global.multiThreadMinFileSize) {
        ...
        for (int i = 0; i < Global.multiThreadCnt; i++) {
            ...
            File fileDownloadPart =
                    new File(fileDownload.getParent(), fileDownload.getName() + ".part" + index);
                                ...
                                public void run() {
                                    ...
                                    try {
                                        ...
                                        if (fileDownloadPart.exists()
                                            ...
                                            // System.out.println("part" + index + "当前已下载: [" +
                                            // offset + "]字节");
                                            raf.seek(offset);
                                        }
                                        specificHeader.put(
                                                "range", String.format(range, min + offset, max));
                                        ...
                                        //							specificHeader.put("range", String.format(range,
                                        // min + offset, max));
                                    ...
                                    // 使用finally块来关闭输入流
                                        ...
                                        synchronized (lock) {
                                            if (count.decrementAndGet() == 0) lock.notifyAll();
                                        }
