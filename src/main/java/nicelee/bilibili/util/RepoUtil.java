...
public class RepoUtil {
    ...
    public static void init(boolean refresh) {
        ...
        try {
            buReader =
                    new BufferedReader(new InputStreamReader(new FileInputStream(fRepo), "utf-8"));
            String avRecord;
    ...
    public static boolean isInRepo(String avRecord) {
        ...
        if (avRecord.contains("-800-")
                || avRecord.contains("-801-")
                || avRecord.startsWith("FFmpeg")) {
            return false;
        }
        if (definitionStrictMode) {
    ...
    static void convertSync() {
        ...
        try {
            buReader =
                    new BufferedReader(
                            new InputStreamReader(new FileInputStream(fRepoNew), "utf-8"));
        ...
        try {
            ...
            buWriter =
                    new BufferedWriter(
                            new OutputStreamWriter(new FileOutputStream(fRepoNew, true), "utf-8"));
            String avRecord;
            int lineCnt = 0;
            while ((avRecord = buReader.readLine()) != null) {
                lineCnt++;
                ...
                if (matcher.find()) {
                    ...
                    if (bvid == null) {
                        ...
                        //						String url =
                        // "https://api.bilibili.com/x/web-interface/view/detail?bvid=&jsonp=jsonp&callback=__jp0&aid="
                        //								+ aid;
                        ...
                        //						try {
                        //							JSONObject infoObj = new JSONObject(callBack.substring(6,
                        // callBack.length() - 2)).getJSONObject("data")
                        //									.getJSONObject("View");
            ...
            File fRepoBackup =
                    ResourcesUtil.sourceOf(
                            "config/repo.config.bk" + System.currentTimeMillis() / 1000);
    ...
    static synchronized void appendRecordToFile(String line) {
