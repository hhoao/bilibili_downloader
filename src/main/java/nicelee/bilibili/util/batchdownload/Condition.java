...
public class Condition {
    ...
    static final HashSet<String> validsOfLeft;
    ...
    public boolean match(ClipInfo clip, int page) {
        switch (left) {
            ...
            case "favTime":
                ...
                throw new RuntimeException("不合法的表达式   " + left + operator + right);
            case "cTime":
                if (clip.getcTime() == 0) throw new RuntimeException("该url类型解析结果不存在cTime字段的信息 ");
                if (operator.equals(">")) {
                    try {
    ...
    public boolean matchUnderlineColon(ClipInfo clip, int page) {
        if (right.equals("_")) {
            ...
            String avRecord =
                    new StringBuilder(clip.getAvId())
                            .append("-")
                            .append(VideoQualityEnum.getQN(Global.menu_qn))
                            .append("-p")
                            .append(clip.getPage())
                            .toString();
