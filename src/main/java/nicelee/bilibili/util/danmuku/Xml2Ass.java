...
public class Xml2Ass {
    ...
    public static void main(String[] args) {
        try {
            ...
            // 从xml读取弹幕
            List<Danmuku> danmuList =
                    xml2AssTool.readXml(
                            "D:\\Workspace\\javaweb-springboot\\BilibiliDown\\release\\download\\惜缘、过去\\【丞相司徒】狭路相逢勇者胜-数星星2_x264.xml");
            // 写入ass文件
            xml2AssTool.writeAss(
                    danmuList,
                    "D:\\Workspace\\javaweb-springboot\\BilibiliDown\\release\\download\\惜缘、过去\\【丞相司徒】狭路相逢勇者胜-pn1-p1-数星星2_x264-80.ass");
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public List<Danmuku> readXml(File xmlFile) throws IOException {
        ...
        BufferedReader reader =
                new BufferedReader(new InputStreamReader(new FileInputStream(xmlFile), "utf-8"));
        List<Danmuku> danmuList = new ArrayList<>();
        Pattern danmukuPattern =
                Pattern.compile(
                        "<d p=\"([^,]*),([^,]*),([^,]*),([^,]*),([^,]*),([^,]*),([^,]*),([^,]*)(?:,[0-9]*)?\">(.*?)</d>");
        ...
        while (line != null) {
            ...
            while (m.find()) {
                ...
                if (fontSizeType <= 12) fontSizeType = 0;
                else if (fontSizeType <= 16) fontSizeType = 1;
                else if (fontSizeType <= 18) fontSizeType = 2;
                else if (fontSizeType <= 25) fontSizeType = 3;
                else if (fontSizeType <= 36) fontSizeType = 4;
                else if (fontSizeType <= 45) fontSizeType = 5;
                else if (fontSizeType <= 64) fontSizeType = 6;
                else continue;
    ...
    public void writeAss(List<Danmuku> danmuList, File assFile) throws IOException {
        ...
        for (int i = 0; i <= 6; i++) {
            String style =
                    String.format(
                            "Style: %s,Microsoft YaHei UI,%d,&H66FFFFFF,&H66FFFFFF,&H66000000,&H66000000,0,0,0,0,100,100,0,0,1,1,1,2,20,20,2,0\r\n",
        ...
        for (Danmuku danmu : danmuList) {
            ...
            if (danmu.getType() == 0) {
                ...
                int endX =
                        0
                                - Danmuku.getFontSize(danmu.getFontSizeType())
                                        * danmuContent.length()
                                        * 2;
                ...
                String line =
                        String.format(
                                "Dialogue: 2,%s,%s,%s,,0000,0000,0000,,{\\move(%d, %d, %d, %d)\\c&H%s&}%s\r\n",
                                startTime,
                                endTime,
                                fontTypeStr,
                                startX,
                ...
                if (normalStartY >= height - maxSize) normalStartY = 0;
                ...
                String line =
                        String.format(
                                "Dialogue: 2,%s,%s,%s,,0000,0000,0000,,{\\an8\\pos(%d, %d)\\c&H%s&}%s\r\n",
                                startTime,
                                endTime,
                                fontTypeStr,
                                startX,
                ...
                if (bottomCenterStartY <= height / 2) bottomCenterStartY = height - maxSize;
                ...
                if (topCenterStartY >= height / 2) topCenterStartY = 0;
            }
        }
        writer.flush();
        writer.close();
