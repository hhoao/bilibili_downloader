...
public class ControllerCookieRefresh {
    ...
    public String index(
            BufferedWriter out, OutputStream outRaw, @Value(key = "pathData") String path) {
    ...
    public String refresh(
            BufferedWriter out, OutputStream outRaw, @Value(key = "postData") String param)
            throws IOException {
        try {
            ...
            // 获取 refresh_csrf
            String url =
                    "https://www.bilibili.com/correspond/1/" + PathDealer.getValue(param, "sufix");
            String html =
                    util.getContent(url, header.getCommonHeaders(), HttpCookies.getGlobalCookies());
            // Logger.println(html);
    ...
    public void makeFileResponse(
            BufferedWriter out, OutputStream outRaw, String path, String contentType) {
        try {
            // System.out.println("请求的path: " + path);
            URL url = this.getClass().getResource("/cookieRefresh" + path);
            if (url != null) {
                InputStream in = this.getClass().getResourceAsStream("/cookieRefresh" + path);
                ResponseUtil.response200OK(out);
