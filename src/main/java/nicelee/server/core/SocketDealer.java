...
public class SocketDealer extends PathDealer implements Runnable {
    ...
    static final Pattern urlPattern =
            Pattern.compile("^(?:GET|POST) ([^ \\?]+)\\??([^ \\?]*) HTTP.*$");
    static final Pattern contentLengthPattern = Pattern.compile("^content-length *: *([0-9]+)$");
    static final Pattern headersPattern = Pattern.compile("^([^:]+) *: *(.+)$");
