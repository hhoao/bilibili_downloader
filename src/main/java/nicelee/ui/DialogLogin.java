...
public class DialogLogin extends JDialog
    ...
    static final String tips = "请输入手机号/邮箱";
    static final Color colorLostFocus = Color.getHSBColor(100, 100, 100);
    static final Color colorGainFocus = Color.white;
    ...
    public void init() {
        ...
        initSysButton(panel);
        ImageIcon icon = new ImageIcon(DialogLogin.class.getResource("/banner.jpg"));
    ...
    private void initSysButton(JPanel panel) {
        ...
        btnClose.setRolloverIcon(new ImageIcon(this.getClass().getResource("/xh.jpg")));
        btnClose.setFocusPainted(false);
        btnClose.setBorderPainted(false);
        btnClose.setContentAreaFilled(false);
        btnClose.setOpaque(true);
        ImageIcon imgx = new ImageIcon(this.getClass().getResource("/x.jpg"));
    ...
    private void queryCaptcha() {
        ...
        if (Global.userName == null
            ...
            try {
                ...
                String url =
                        String.format(
                                "http://localhost:%d/static/index.html?token=%s&gt=%s&challenge=%s",
                                Global.serverPort, token, gt, challenge);
                final boolean browseSupported =
                        Desktop.getDesktop().isSupported(Desktop.Action.BROWSE);
                if (browseSupported) Desktop.getDesktop().browse(new URI(url));
                else {
    ...
    public String login(String token, String challenge, String validate, String seccode) {
        ...
        isLogging = true;
        String tips =
                inl.login(Global.userName, Global.password, token, challenge, validate, seccode);
        if (tips == null) {
            lbTips.setText("登录成功");
            Global.isLogin = true;
    ...
    public void focusLost(FocusEvent e) {
        if (e.getSource() == jtUserName) {
            ...
            if (content.isEmpty()) jtUserName.setText(tips);
            else Global.userName = content;
        } else if (e.getSource() == jtPassword) {
            String content = String.valueOf(jtPassword.getPassword());
            if (content.isEmpty()) jtPassword.setText("123456");
            else Global.password = content;
        }
        JComponent target = (JComponent) e.getSource();
