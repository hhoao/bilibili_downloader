...
public class Global {
    ...
    @Config(
            ...
            defaultValue = "true",
            eq_true = "default",
            valids = {"default", "system"})
    public static boolean themeDefault;

    @Config(
            key = "bilibili.button.style",
            note = "Button样式",
            defaultValue = "true",
            eq_true = "design",
            valids = {"design", "default"})
    public static boolean btnStyle = true;

    @Config(
            key = "bilibili.lockCheck",
            note = "防止多开",
            defaultValue = "false",
            valids = {"true", "false"})
    ...
    @Config(
            key = "bilibili.alert.isAlertIfDownloded",
            note = "提交已完成的视频时是否弹出提示",
            defaultValue = "true",
            valids = {"true", "false"})
    ...
    @Config(
            key = "bilibili.sysTray.minimizeToSystray",
            note = "点击最小化按钮到托盘",
            defaultValue = "true",
            valids = {"true", "false"})
    ...
    @Config(
            key = "bilibili.sysTray.closeToSystray",
            note = "点击关闭按钮到托盘",
            defaultValue = "false",
            valids = {"true", "false"})
    ...
    @Config(
            key = "bilibili.tab.display.previewPic",
            note = "Tab页面自动显示作品预览图",
            defaultValue = "on",
            eq_true = "on",
            valids = {"on", "off"})
    ...
    @Config(
            key = "bilibili.menu.tab.promptBeforeCloseAllTabs",
            note = "点击【关闭全部Tab页】后，弹出确认框",
            defaultValue = "true",
            valids = {"true", "false"})
    ...
    @Config(
            key = "bilibili.download.thumbUp",
            note = "下载完成后给作品点赞",
            defaultValue = "false",
            valids = {"true", "false"})
    public static volatile boolean thumbUpAfterDownloaded; // 下载完成后是否给作品点赞

    @Config(
            key = "bilibili.download.playSound",
            note = "全部任务完成后播放提示音",
            defaultValue = "false",
            valids = {"true", "false"})
    ...
    @Config(
            key = "bilibili.download.retry.reloadDownloadUrl",
            note = "重试时，重新查询下载链接",
            defaultValue = "false",
            valids = {"true", "false"})
    ...
    @Config(
            key = "bilibili.download.urlValidPeriod",
            note = "下载url的有效时长(min)",
            defaultValue = "90",
            multiply = 60000)
    ...
    @Config(
            ...
            defaultValue = "0",
            valids = {"0", "1", "2"},
            note = "优先下载格式, 0-m4s,1-flv,2-mp4")
    public static int downloadFormat = MP4; // 优先下载格式，如不存在该类型的源，那么将默认转为下载另一种格式

    @Config(
            key = "bilibili.dash.download.mode",
            defaultValue = "0",
            valids = {"0", "1", "2"},
            note = "DASH下载模式: 0-下载音视频,1-仅视频,2-仅音频")
    public static DownloadModeEnum downloadMode = DownloadModeEnum.All;

    @Config(
            key = "bilibili.dash.video.codec.priority",
            defaultValue = "7, 12, 13",
            note = "视频编码优先级(默认),AV1:13,HEVC:12,AVC:7,随意-1")
    public static int[] videoCodecPriority = {7, 12, 13};

    @Config(
            key = "bilibili.dash.video.codec.priority.map",
            defaultValue = "80:7, 12, 13| 64:7, 12, 13",
            note = "视频编码优先级(区分清晰度),AV1:13,HEVC:12,AVC:7,随意-1")
    public static HashMap<Integer, int[]> videoCodecPriorityMap;

    @Config(
            key = "bilibili.dash.audio.quality.priority",
            defaultValue = "30280, 30232, 30216, -1, 30251, 30250",
            note = "音频编码优先级,30216:64K, 30232:132K, 30280:192K, 随意-1")
    public static int[] audioQualityPriority = {30280, 30232, 30216, -1};

    @Config(
            key = "bilibili.dash.checkUrl",
            note = "查询DASH方式的下载链接时，检查链接有效性",
            defaultValue = "false",
            valids = {"true", "false"})
    public static boolean checkDashUrl = false;

    @Config(
            key = "bilibili.download.forceHttp",
            note = "强制将音视频下载地址的https转为http(PCDN除外)",
            defaultValue = "false",
            valids = {"true", "false"})
    public static boolean forceHttp = false;

    @Config(
            key = "bilibili.download.host.forceReplace",
            note = "强制替换音视频下载服务器host",
            defaultValue = "false",
            valids = {"true", "false"})
    public static boolean forceReplaceUposHost = false;

    @Config(
            key = "bilibili.download.host.forceReplace.pattern",
            note = "功能开启时，匹配该规则的链接会被替换host",
            defaultValue = "https?://[^/]+/upgcxcode")
    public static String forceReplaceUrlPattern = "^https?://[^/]+/upgcxcode";

    @Config(
            key = "bilibili.download.host.alternative",
            note = "自定义的下载服务器host",
            defaultValue = "upos-sz-estghw.bilivideo.com")
    public static String altHost = "upos-sz-mirror08c.bilivideo.com";

    @Config(
            key = "bilibili.savePath",
            note = "保存路径",
    ...
    @Config(
            key = "bilibili.download.period.between.download",
            note = "每个下载任务完成后的等待时间(ms)",
            defaultValue = "0",
            multiply = 1)
    public static long sleepAfterDownloadComplete;

    @Config(
            key = "bilibili.download.period.between.query",
            note = "每个关于下载的查询任务完成后的等待时间(ms)",
            defaultValue = "0",
            multiply = 1)
    ...
    @Config(
            key = "bilibili.download.multiThread.count",
            note = "单个下载任务开启线程数(0,1为不开启多线程)",
            defaultValue = "0")
    public static int multiThreadCnt; // 多线程下载开启的线程数 0为不开启多线程下载

    @Config(
            key = "bilibili.download.multiThread.minFileSize",
            note = "文件大小小于阈值(整数, 单位MB)，则不开启多线程下载",
            defaultValue = "0",
            multiply = 1024 * 1024)
    public static long multiThreadMinFileSize; // 文件大小小于阈值(MB)，则不开启开启多线程下载

    @Config(
            key = "bilibili.download.multiThread.singlePattern",
            note = "url匹配该正则，则不开启多线程下载",
            defaultValue = "github|ffmpeg|\\.jpg|\\.png|\\.webp|\\.xml")
    public static Pattern singleThreadPattern; // url匹配该正则，则不开启开启多线程下载

    @Config(
            key = "bilibili.repo",
            note = "创建下载任务前判断是否已下载(会自动保存下载的BV号)",
            defaultValue = "true",
            eq_true = "on",
            valids = {"on", "off"})
    ...
    @Config(
            key = "bilibili.repo.save",
            note = "在不判断已下载时仍然保存下载的BV号",
            defaultValue = "true",
            eq_true = "on",
            valids = {"on", "off"})
    ...
    @Config(
            key = "bilibili.name.format",
            note = "自定义下载文件名称",
            defaultValue = "(:listName listName-)avTitle-pDisplay-clipTitle-qn")
    public static String formatStr;

    @Config(
            key = "bilibili.name.format.clipTitle.allowNull",
            note = "自定义下载名称时，若avTitle等于clipTitle，将clipTitle置空",
            defaultValue = "false",
            eq_true = "true",
            valids = {"true", "false"})
    ...
    @Config(
            key = "bilibili.name.doAfterComplete",
            note = "下载完成后自动重命名",
            defaultValue = "true",
            valids = {"true", "false"})
    ...
    @Config(
            key = "bilibili.repo.definitionStrictMode",
            note = "是否将同一视频不同清晰度看作不同任务",
            defaultValue = "false",
            eq_true = "on",
            valids = {"on", "off"}) /* 存在某一清晰度后, 在下载另一种清晰度时是否判断已完成 */
    ...
    @Config(
            key = "bilibili.download.batch.plan",
            note = "按计划分配每次一键下载任务之间的间隔",
            defaultValue =
                    "06:00~02:00=>r(300,480); 02:00~06:00=>~06:00+r(0,360); 00:00~00:00=>r(600,600)")
    public static String batchDownloadPlan;

    @Config(
            key = "bilibili.download.batch.plan.runOnStartup",
            note = "在程序启动时，按计划进行周期性批量下载",
            defaultValue = "false",
            valids = {"true", "false"})
    public static boolean batchDownloadRbyRRunOnStartup;

    @Config(
            key = "bilibili.download.batch.config.name",
            note = "一键下载配置的默认名称",
            defaultValue = "batchDownload.config")
    public static String batchDownloadConfigName;

    @Config(
            key = "bilibili.download.batch.config.name.pattern",
            note = "一键下载配置名称的匹配正则表达式",
            defaultValue = "^batchDownload.*\\.config$")
    public static Pattern batchDownloadConfigNamePattern;
    // 登录相关
    @Config(
            key = "bilibili.login.cookie.tryRefreshOnStartup",
            note = "每次程序打开时尝试刷新Cookie",
            defaultValue = "false",
            valids = {"true", "false"})
    public static boolean tryRefreshCookieOnStartup;

    @Config(
            key = "bilibili.login.cookie.refresh.runWASMinBrowser",
            defaultValue = "false",
            valids = {"true", "false"})
    ...
    @Config(
            key = "bilibili.user.delete",
            defaultValue = "true",
            valids = {"true", "false"})
    public static boolean deleteUserFile;

    @Config(
            key = "bilibili.user.login",
            note = "登录方式 qr/pwd/sms",
            defaultValue = "qr",
            valids = {"pwd", "qr", "sms"}) // note = "登录方式",
    public static String loginType; // 登录方式
    // @Config(key = "bilibili.user.login.pwd", defaultValue = "false", eq_true = "auto", valids = {
    // "auto", "manual" })
    // public static boolean pwdAutoLogin; // 使用用户名密码自动登录方式
    // @Config(key = "bilibili.user.login.pwd.autoCaptcha", defaultValue = "false", valids = {
    // "true", "false" })// note = "用户名密码登录时是否自动尝试解析验证码",
    ...
    @Config(
            key = "bilibili.info.query.strategy",
            note = "查询可用清晰度时，使用的策略",
            defaultValue = "returnFixedValue",
            valids = {"tryNormalTypeFirst", "judgeTypeFirst", "returnFixedValue"})
    public static String infoQueryStrategy;
    // 临时文件相关
    @Config(
            key = "bilibili.restrictTempMode",
            defaultValue = "true",
            eq_true = "on",
            valids = {"on", "off"})
    ...
    @Config(
            key = "bilibili.download.update.sources.active",
            note = "生效的更新源",
            defaultValue = "Github")
    ...
    @Config(
            key = "bilibili.download.ffmpeg.sources.active",
            note = "生效的ffmpeg源",
            defaultValue = "Github")
    public static String ffmpegSourceActive;
    // FFMPEG 路径
    @Config(
            key = "bilibili.ffmpegPath",
            note = "ffmpeg路径",
            defaultValue = "ffmpeg",
            pathType = "file")
    ...
    @Config(
            key = "bilibili.dash.ffmpeg.command.merge",
            note = "ffmpeg音视频合并命令",
            defaultValue =
    ...
    @Config(
            key = "bilibili.dash.ffmpeg.command.transAudioOnly",
            note = "ffmpeg音频转换命令",
            defaultValue =
    ...
    @Config(
            key = "bilibili.flv.ffmpeg",
            note = "FLV合并时是否调用ffmpeg",
            defaultValue = "false",
            valids = {"true", "false"})
    public static boolean flvUseFFmpeg = false;

    @Config(
            key = "bilibili.alert.ffmpegFail",
            note = "ffmpeg合并失败是否抛出异常",
            defaultValue = "true",
            valids = {"true", "false"})
    public static boolean alertIfFFmpegFail = true;

    @Config(
            key = "bilibili.cmd.debug",
            note = "调用外部命令时是否显示输出",
            defaultValue = "false",
            valids = {"true", "false"})
    ...
    @Config(
            key = "bilibili.alert.qualityUnexpected",
            note = "当遇到不期望的480P视频时是否抛出异常",
            defaultValue = "true",
            valids = {"true", "false"})
    ...
    @Config(
            key = "bilibili.https.allowInsecure",
            note = "跳过证书验证",
            defaultValue = "false",
            valids = {"true", "false"})
    ...
    @Config(
            key = "bilibili.userAgent.pc",
            note = "HTTP请求使用的UserAgent(PC Web)",
            defaultValue =
    ...
    @Config(
            key = "bilibili.userAgent.pc.fingerprint",
            note = "浏览器指纹(取自cookie buvid_fp)",
            defaultValue = "a8bad806241b0b0f7add1024fbd701fa")
    public static String userAgentFingerprint = "";

    @Config(
            key = "bilibili.userAgent.pc.payload",
            note = "截取自api请求https://api.bilibili.com/x/internal/gaia-gateway/ExClimbWuzhi",
            defaultValue = "")
    public static String userAgentPayload = "";

    @Config(
            key = "bilibili.github.token",
            note = "Github API鉴权token",
            defaultValue = "",
            warning = false)
    public static String githubToken; // 一般情况下，用于Github API鉴权, 暂只用于更新Beta版本

    public static final HashMap<String, String> settings = new LinkedHashMap<>();
    public static final HashSet<String> settingsMustCreateManualy = new HashSet<>();
    ...
    public static void init() {
        for (Field field : Global.class.getDeclaredFields()) {
            ...
            if (config != null) {
                ...
                if (checkValid(valueFromSettings, config.valids())) {
                    ...
                    if (config.warning())
                        System.err.printf(
                                "%s 值为 %s, 不在合法范围内 %s, 将使用默认值\n",
                                config.key(), valueFromSettings, Arrays.asList(config.valids()));
        ...
        if (Global.backgroundImg == null)
            Global.backgroundImg = new ImageIcon(Global.class.getResource("/background.png"));
        ...
        //  设置System Property
        String sysPropJreTag =
                "bilibili.system.properties.jre" + System.getProperty("java.specification.version");
        String sysProp = Global.settings.get(sysPropJreTag);
        if (sysProp != null) {
            String sysPropOverrideStr =
                    Global.settings.getOrDefault(sysPropJreTag + ".override", "false");
            ...
            for (String keyValuePair : keyValuePairs) {
                ...
                if (params.length == 2) {
                    if (sysPropOverride || System.getProperty(params[0]) == null) {
                        ...
                        else System.setProperty(params[0], params[1].trim());
                    }
                }
            }
        }
    ...
    private static void setValue(Field field, String value, boolean isDefaultValue, Config config) {
        try {
            ...
            if (field.getType().equals(String.class)) {
                if (isDefaultValue && value.isEmpty()) field.set(null, null);
                ...
                if (isDefaultValue) field.set(null, "true".equalsIgnoreCase(value));
                else field.set(null, config.eq_true().equalsIgnoreCase(value));
