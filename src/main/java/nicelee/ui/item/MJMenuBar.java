...
public class MJMenuBar extends JMenuBar {
    ...
    private void init() {
        ...
        JMenu dUseRepoMenuItem =
                new MJMenuWithRadioGroupBuilder("下载前先查询记录?", "查询", "不查询") {
                    @Override
                    public void onItemSelected(int itemIndex, JRadioButtonMenuItem item) {
        ...
        JMenu dDashDownTypeMenuItem =
                new MJMenuWithRadioGroupBuilder("下载模式(DASH)", "全部", "仅视频", "仅音频") {
                    @Override
                    public void onItemSelected(int itemIndex, JRadioButtonMenuItem item) {
        ...
        JMenu dTypeReDownloadMenuItem =
                new MJMenuWithRadioGroupBuilder("下载重试策略", "使用之前的url", "重新查询url") {
                    @Override
                    public void init(JRadioButtonMenuItem[] menuItems) {
        ...
        JMenu dForceReplaceHostMenuItem =
                new MJMenuWithRadioGroupBuilder("音视频链接替换host?", "替换", "不替换") {
                    @Override
                    public void onItemSelected(int itemIndex, JRadioButtonMenuItem item) {
        ...
        JMenu dQNMenuItem =
                new MJMenuWithRadioGroupBuilder("优先清晰度", qnSelections) {
        ...
        JMenu dQNQueryStrategyMenuItem =
                new MJMenuWithRadioGroupBuilder("可用清晰度查询策略", straOptionTips) {
        ...
        JMenu dBatchDownMenuItem =
                new MJMenuWithRadioGroupBuilder("一键下载配置", configFiles) {
        ...
        JMenu dUpdateMenuItem =
                new MJMenuWithRadioGroupBuilder(
                        "更新源选择", Global.updateSourceAvailable.split("\\|")) {
                    @Override
                    public void onItemSelected(int itemIndex, JRadioButtonMenuItem item) {
        ...
        JMenu dFFmpegMenuItem =
                new MJMenuWithRadioGroupBuilder(
                        "FFMPEG源选择", Global.ffmpegSourceAvailable.split("\\|")) {
                    @Override
                    public void onItemSelected(int itemIndex, JRadioButtonMenuItem item) {
        ...
        if (Global.githubToken != null && !Global.githubToken.isEmpty()) {
            ...
            updateBetaMenuItem.addActionListener(
                    new ActionListener() {
                        @Override
                        public void actionPerformed(ActionEvent e) {
                            VideoInfo avInfo =
                                    new INeedAV()
                                            .getVideoDetail("BilibiliDown.PreRelease", 0, false);
                            DownloadRunnable downThread =
                                    new DownloadRunnable(avInfo, avInfo.getClips().get(1234L), 0);
                            Global.queryThreadPool.execute(downThread);
                        }
                    });
        }
        // 一键下载
        batchDownload.addActionListener(
                new ActionListener() {
                    @Override
                    public void actionPerformed(ActionEvent e) {
        ...
        // 按计划一键下载
        batchDownloadRbyR.addActionListener(
                new ActionListener() {
                    @Override
                    public void actionPerformed(ActionEvent e) {
        ...
        reloadConfig.addActionListener(
                new ActionListener() {
                    @Override
                    public void actionPerformed(ActionEvent e) {
        ...
        // 保存下载页的所有任务
        saveDownloading.addActionListener(
                new ActionListener() {
                    @Override
                    public void actionPerformed(ActionEvent e) {
                        ...
                        try (BufferedWriter writer =
                                new BufferedWriter(
                                        new OutputStreamWriter(
                                                new FileOutputStream(downloadingTasks), "utf-8"))) {
        ...
        // 加载保存的任务到下载页
        loadDownloading.addActionListener(
                new ActionListener() {
                    @Override
                    public void actionPerformed(ActionEvent e) {
                        ...
                        try (BufferedReader reader =
                                new BufferedReader(
                                        new InputStreamReader(
                                                new FileInputStream(downloadingTasks), "utf-8"))) {
                            String line;
        ...
        // 批量下载
        doMultiDownMenuItem.addActionListener(
                new ActionListener() {
                    @Override
                    public void actionPerformed(ActionEvent e) {
        ...
        refreshCookie.addActionListener(
                new ActionListener() {
                    @Override
                    public void actionPerformed(ActionEvent e) {
                    ...
                    public void actionPerformed(ActionEvent e) {
                        if (Global.downloadTab.activeTask > 0) {
                            JOptionPane.showMessageDialog(
                                    null, "当前仍然存在下载任务！", "请注意!!", JOptionPane.WARNING_MESSAGE);
                        } else if (!Global.isLogin) {
                            JOptionPane.showMessageDialog(
                                    null, "当前没有登录！", "请注意!!", JOptionPane.WARNING_MESSAGE);
                        } else {
                            API.logout();
                                        ...
                                        new Runnable() {
                                            ...
                                            public void run() {
                                                ...
                                                try {
                                                    ...
                                                    JOptionPane.showMessageDialog(
                                                            null,
                                                            "出现了异常，异常原因为：" + e1.toString(),
                                                            "异常",
                                                            JOptionPane.PLAIN_MESSAGE);
                                                }
                                                frame.setTitle(
                                                        frame.getTitle().replace(" 版本更新中", ""));
                                            }
                                        },
                                        "更新线程")
                                .start();
