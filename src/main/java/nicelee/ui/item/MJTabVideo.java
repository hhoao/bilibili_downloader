...
public class MJTabVideo extends TabVideo { // implements MouseListener, ActionListener
        ...
        btnNextPage.addActionListener(
                new ActionListener() {
                    ...
                    public void actionPerformed(ActionEvent e) {
                        ...
                        final TabVideo tab =
                                new MJTabVideo(jTabbedpane, label, modifiedSearchContent);
                        jTabbedpane.addTab("作品页", tab);
                        jTabbedpane.setTabComponentAt(jTabbedpane.indexOfComponent(tab), label);
                        GetVideoDetailThread th =
                                new GetVideoDetailThread(tab, modifiedSearchContent);
                        th.start();
    ...
    public void mousePressed(MouseEvent e) {
        if (e.getSource() == label) {
            if (e.getButton() == MouseEvent.BUTTON3) pop.show(label, e.getX(), e.getY());
        } else {
