...
public class MJTitleBar extends JPanel implements MouseListener, MouseMotionListener {
    ...
    private void initTitle() {
        ...
        if (frame.getIconImage() != null) {
            ...
            ImageIcon iconImg =
                    new ImageIcon(
                            frame.getIconImage().getScaledInstance(25, 23, Image.SCALE_DEFAULT));
    ...
    private void initButton() {
        // 添加最小化按钮
        btnMin = new JButton();
        btnMin.setRolloverIcon(new ImageIcon(this.getClass().getResource("/_h.jpg")));
        btnMin.setFocusPainted(false);
        btnMin.setBorderPainted(false);
        btnMin.setContentAreaFilled(false);
        ImageIcon img_ = new ImageIcon(this.getClass().getResource("/_.jpg"));
        btnMin.setIcon(img_);
    ...
    public void mouseClicked(MouseEvent e) {
        if (e.getSource() == btnClose) {
            if (frame instanceof FrameMain
                    && SysTray.isSysTrayInitiated()
                    && Global.closeToSystray) {
                ...
                if (frame instanceof FrameMain && Global.downloadTab.activeTask > 0) {
                    ...
                    String msg =
                            String.format(
                                    "当前仍有 %d 个任务在下载/转码，正在转码的文件退出后可能丢失或异常，确定要退出吗？",
    ...
    public void mouseDragged(MouseEvent e) {
        ...
        if (pressedPoint != null && e.getSource() != btnClose && e.getSource() != btnMin) {
            ...
            if (newX == frame.getX() && newY == frame.getY()) return;
