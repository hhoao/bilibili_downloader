...
public class BatchDownloadThread extends Thread {
                ...
                while (!stopFlag) {
                    ...
                    for (ClipInfo clip : clips) {
                        ...
                        if (batch.matchStopCondition(clip, page)) {
                            // 判断边界BV是否要下载
                            if (batch.isIncludeBoundsBV()
                                    && batch.matchDownloadCondition(clip, page)) {
                                addTask(clip);
    ...
    public void checkValid(File f) throws IOException, URISyntaxException {
        if (f == null || !f.exists()) {
            String docsUrl =
                    "https://nICEnnnnnnnLee.github.io/BilibiliDown/guide/advanced/quick-batch-download";
