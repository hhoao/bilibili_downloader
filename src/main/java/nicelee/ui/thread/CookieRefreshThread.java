...
public class CookieRefreshThread extends Thread {
    ...
    public void run() {
        ...
        if (HttpCookies.getGlobalCookies() == null) {
            if (showTips) JOptionPaneManager.showMsgWithNewThread("消息", "当前未登录");
        ...
        if (HttpCookies.getRefreshToken() == null) {
            if (showTips) JOptionPaneManager.showMsgWithNewThread("消息", "当前的登录凭证为旧版本生成，无法刷新cookie");
            return;
        }
        // 判断需不需要刷新
        String url =
        ...
        String csrfInfo =
                util.getContent(url, headers.getCommonHeaders(), HttpCookies.getGlobalCookies());
        ...
        if (!needRefresh) {
            if (showTips) JOptionPaneManager.showMsgWithNewThread("消息", "当前cookie无需刷新");
            API.uploadFingerprint();
            Logger.println("Fingerprint上传完毕");
            return;
        }
        if (Global.runWASMinBrowser) {
            ...
            String browseUrl =
                    String.format(
                            "http://localhost:%d/cookieRefresh/index.html?timestamp=%d",
            ...
            try {
                ...
                String html =
                        util.getContent(
                                path, headers.getCommonHeaders(), HttpCookies.getGlobalCookies());
                ...
                e.printStackTrace();
                if (showTips) JOptionPaneManager.showMsgWithNewThread("消息", "刷新cookie出现错误");
                return;
            }
        }
        // 调用刷新Cookie的API
        ...
        String tips =
                inl.refreshCookie(
                        HttpCookies.getCsrf(), refreshCsrf, HttpCookies.getRefreshToken());
        if (showTips)
            if (tips != null) {
                Logger.println(tips);
