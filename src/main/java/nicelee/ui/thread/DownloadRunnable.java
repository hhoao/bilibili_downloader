...
public class DownloadRunnable implements Runnable {
    ...
    static final String MSG_VIDEO_DOWNLOADED =
            "您已经下载过视频 %s\n如果想继续下载:\n"
    ...
    public void run() {
        try {
            ...
            JOptionPaneManager.alertErrMsgWithNewThread(
                    "发生了预料之外的错误", ResourcesUtil.detailsOfException(e));
    ...
    public void download() {
        ...
        if (Global.useRepo && RepoUtil.isInRepo(record)) {
            JOptionPaneManager.showMsgWithNewThread(
                    "提示", String.format(MSG_VIDEO_DOWNLOADED, record));
        ...
        if (!ResourcesUtil.isPicture(avid)) {
            urlQuery =
                    iNeedAV.getInputParser(avid)
                            .getVideoLink(avid, cid, qn, Global.downloadFormat); // 该步含网络查询， 可能较为耗时
        ...
        if (!Global.downLoadThreadPool.isShutdown()) {
            Global.downLoadThreadPool.execute(
                    new DownloadRunnableInternal(downPanel, System.currentTimeMillis(), false, 0));
        }
