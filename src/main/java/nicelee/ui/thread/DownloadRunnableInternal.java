...
public class DownloadRunnableInternal implements Runnable {
     ...
     *
     * <p>为了防止该任务在线程池的队列里面等太久，需要urlTimestamp来标定url的有效性，以判断是否重新查询url
     *
     ...
     */
    public DownloadRunnableInternal(
            DownloadInfoPanel downPanel,
            long urlTimestamp,
    ...
    public void run() {
        try {
            ...
            if (!ResourcesUtil.isPicture(avid)) { // 不是图片类型(该类型没啥好重新获取的)
                ...
                if (invokeByContinueTask && Global.reloadDownloadUrl) {
                    ...
                    if (deltaTime > Global.urlValidPeriod) {
                        Logger.printf(
                                "下载url距离上次查询已经过了超过%d min，重新查询下载链接", Global.urlValidPeriod / 60000L);
                        shouldReQuery = true;
                    }
                }
            ...
            // 开始下载
            if (iNeedAV.downloadClip(
                    validUrl, avid, iNeedAV.getInputParser(avid).getVideoLinkQN(), page)) {
                // 下载成功后保存到仓库
