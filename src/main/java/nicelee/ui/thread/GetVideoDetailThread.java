    ...
    public void run() {
        try {
            ...
            if (Global.autoDisplayPreviewPic) {
                if (clips.size() == 0) video.setCurrentDisplayPic(avInfo.getVideoPreview());
                else video.setCurrentDisplayPic(clips.iterator().next().getPicPreview());
                try {
                    ...
                    imag1 =
                            new ImageIcon(
                                    imag1.getImage()
                                            .getScaledInstance(700, 460, Image.SCALE_SMOOTH));
