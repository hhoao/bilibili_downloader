...
public class LoginThread extends Thread {
    ...
    private void QRLogin(INeedLogin inl) {
        /**
         * 1. 访问 Get 访问 https://passport.bilibili.com/qrcode/getLoginUrl 获取 oauthKey ==> 链接 ==> 二维码
         */
        ...
        /** 2. 周期性Post 访问 https://passport.bilibili.com/qrcode/getLoginInfo 直至扫码成功 成功后保存Cookie */
        long start = System.currentTimeMillis();
        while (!Global.isLogin
                && Global.needToLogin
    ...
    public void initUserInfo(INeedLogin inl) {
        ...
        try {
            String favUrl =
                    "https://api.bilibili.com/medialist/gateway/base/created?pn=1&ps=100&is_space=0&jsonp=jsonp&up_mid="
                            + inl.user.getUid();
            HttpRequestUtil util = new HttpRequestUtil();
            String jsonStr =
                    util.getContent(
                            favUrl,
                            new HttpHeaders().getAllFavListHeaders(inl.user.getUid()),
                ...
                for (int i = 0; i < list.length(); i++) {
                    ...
                    FavList fav =
                            new FavList(
                                    favlist.getLong("mid"),
                                    favlist.getLong("id"),
                                    favlist.getInt("media_count"),
                                    favlist.getString("title"));
