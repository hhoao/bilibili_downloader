...
public class MonitoringThread extends Thread {
    ...
    public void run() {
        ...
        if (Global.playSoundAfterMissionComplete) Audio.init();
        ...
        while (true) {
                ...
                try {
                    ...
                    if (file != null) {
                        ...
                        if (Global.doRenameAfterComplete
                                && downloader.currentStatus() == StatusEnum.SUCCESS) {
                            path =
                                    path.replaceFirst(
                    ...
                    switch (downloader.currentStatus()) {
                        case SUCCESS:
                            ...
                            if (dp.getBackground() != lightGreen) {
                                String fileSize =
                                        IDownloader.transToSizeStr(downloader.sumTotalFileSize());
                                dp.getLbCurrentStatus()
                                        .setText(genTips("%d/%d 下载完成. ", downloader));
                                dp.getLbDownFile().setText("文件大小: " + fileSize);
                                dp.getBtnControl().setVisible(false);
                                dp.setBackground(lightGreen);
                                // TODO 将成功的任务状态记入
                                BatchDownloadRbyRThread.taskSucceed(
                                        dp.getClipInfo(),
                                        formattedTitle,
                                        fileSize,
                                        "" + dp.getRealqn());
                        ...
                        case FAIL:
                            ...
                            if (dp.getFailCnt() == MAX_FAIL_CNT) {
                                if (!dp.getLbCurrentStatus().getText().endsWith("下载异常. ")) {
                                    dp.getLbCurrentStatus()
                                            .setText(genTips("%d/%d 下载异常. ", downloader));
                                ...
                                pauseTaskCanRetry++;
                                dp.getLbCurrentStatus()
                                        .setText(String.format("下载异常. 尝试重连 %d ", dp.getFailCnt()));
                        ...
                        case PROCESSING:
                            ...
                            dp.getLbDownFile()
                                    .setText(
                                            "文件大小: "
                                                    + IDownloader.transToSizeStr(
                                                            downloader.sumTotalFileSize()));
                        ...
                        case NONE:
                            if (dp.stopOnQueue) {
                                pauseTask++;
                                dp.getLbCurrentStatus()
                                        .setText(genTips("%d/%d 人工停止. ", downloader));
                                dp.getLbDownFile()
                                        .setText(genSizeCntStr("文件%d进度： %s/%s", downloader));
                                dp.getBtnControl().setText("继续下载");
                                dp.getBtnControl().setVisible(true);
                                if (dp.getBackground() != lightGreen
                                        && dp.getBackground() != lightPink) {
                        ...
                        case DOWNLOADING:
                            ...
                            int downSize =
                                    (int)
                                            (downloader.sumDownloadedFileSize()
                                                    - dp.getLastCnt()); // byte
            ...
            Global.downloadTab.refreshStatus(
                    totalTask,
                    activeTask + pauseTaskCanRetry,
                    pauseTask - pauseTaskCanRetry,
                    doneTask,
                    queuingTask);
            ...
            if (Global.playSoundAfterMissionComplete
                    ...
                    && activeTask == 0
                    && pauseTaskCanRetry == 0) Audio.play();
    ...
    String genTips(String format, IDownloader downloader) {
        String tips;
        tips = String.format(format, downloader.currentTask(), downloader.totalTaskCount());
        return tips;
    }
